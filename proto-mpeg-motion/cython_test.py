...
t=time.time()
proto_mpeg_x.encode_video(files[0:nFrames], 'video.bin', 1)
print('Encoded a frame in %.3f seconds'%((time.time()-t)/nFrames))
'''
...
print('Decoded a frame in %.3f seconds'%((time.time()-t)/nFrames))

print(np.shape(video))

ec504viewer.view_sequence_mtezcan(video, 1)
'''
